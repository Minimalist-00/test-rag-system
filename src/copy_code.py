...
###制約
    ...
    ・表形式の情報を返す場合は、htmlテーブルとして返します。Markdown形式では返さないでください。
    ・各情報源には、名前の後にコロンと実際の情報が続きます。回答で使用する各事実について、常に情報源名を含めてください。情報源を参照するには、各Content情報の前段にあるfilepathとchunk-idの情報を反映してください。角かっこを使用してください。情報源参照ルール：[filepath]-[chunk-id] 　情報源出力例：[info1.txt]-[1]
    ・情報源を組み合わせないでください。各情報源を個別にリストしてください。例：[info1.txt]-[1],[info1.txt]-[2]
